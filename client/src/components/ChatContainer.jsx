...
const Container = () => {
  ...
  return selectedUser ? (
        ...
        ${msg.senderId !== authUser._id && 'flex-row-reverse'}`}>
           {msg.image ? (
            ...
            <p className={`p-2 max-w-[200px] md:text-sm font-light
              border-gray-700 rounded-lg overflow-hidden mb-8 break-all
      ...
      <div className='absolute bottom-0 left-0 right-0 flex items-center gap-3 p-3'>
        <div className=' flex-1 flex items-center bg-gray-100/12 px-3 rounded-full'>
          <input onChange={(e)=>setInput(e.target.value)} value={input} onKeyDown={(e)=>
           ...
           className='flex-1 text-sm rounded-lg border-none outline-none p-3 
    ...
    <div className='flex flex-col items-center justify-center
     gap-5 text-gray-500 bg-white/10 max-md:hidden' >
