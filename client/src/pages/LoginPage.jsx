    ...
    gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
      ...
      flex-col gap-6 rounded-lg shadow-lg'>
        ...
        {!isDataSubmitted && (
            ...
            rounded-md focus:utline-none focus:ring-2 focus:ring-indigo-500' />

            <input onChange={(e)=>setPassword(e.target.value)} value={password} 
            type="password" placeholder='Password' required className='p-2 border 
            border-gray-500 rounded-md focus:utline-none focus:ring-2 
        ...
        <button type='submit' className='py-3 bg-gradient-to-r from-purple to-violet-600 text-white
        ...
        <div className='flex flex-col gap-2'>
          ...
          (
            ...
            <span onClick={()=>{setCurrState("Login"); isDataSubmitted(false)}} 
            className='text-violet-500 font-medium cursor-pointer'>Login here</span></p>
          ) : (
            <p className='text-sm text-gray-600'>Create an account? 
            <span onClick={()=>{setCurrState("Sign Up")}} 
            className='text-violet-500 font-medium cursor-pointer'>Click here</span></p>
          )}
        </div>
